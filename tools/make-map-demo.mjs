...
let HI_TEX_W = 16384;     // recomputed from renderer.capabilities once we have it
let equirect;
...
function initGL() {
  ...
  // Cap the upgrade resolution at whatever the GPU supports (cards on phones
  // often max out at 4096 or 8192; desktop usually goes to 16384+).
  HI_TEX_W = Math.min(16384, renderer.capabilities.maxTextureSize || 8192);
  ...
  globeCanvas.addEventListener('wheel', e => {
    ...
    lodTimer = setTimeout(() => {
      const want = camera.position.z < 1.7 ? HI_TEX_W : 8192;
      if (want !== TEX_W) {
