...
function render(name) {
  ...
  if (borderLines.length) drawBorders(proj, xm, ym, W, H);
}

// ── Vektorgränserna (samma linjer som globen) ritas ovanpå varpen ──
let borderLines = [];
async function loadBorders() {
  try {
    const gj = await (await fetch('assets/art-borders.json')).json();
    borderLines = gj.features[0].geometry.coordinates;
  } catch (e) { console.warn('kunde inte läsa art-borders.json', e); }
}
function drawBorders(proj, xm, ym, W, H) {
  ...
  for (const line of borderLines) {
    ...
    for (const [lng, lat] of line) {
      ...
      const px = (x + xm) / (2 * xm) * W;
      const py = (ym - y) / (2 * ym) * H;
      // bryt vid antimeridianhopp
      if (prev && Math.abs(px - prev) > W / 2) { ctx.moveTo(px, py); prev = px; continue; }
...
await Promise.all([loadMosaic(), loadBorders()]);
