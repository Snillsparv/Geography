...
Lossy WebP/JPEG scans carry compression speckle that explodes into thousands of
tiny colour regions when vectorized. This step:
  ...
  3. quantizes to a clean adaptive palette (detail-preserving by default),

and writes a cleaned PNG to a staging directory, mirroring the input layout.
...
# Tunables (kept generous so we never sacrifice detail richness).
COLORS = int(os.environ.get("VEC_COLORS", "16"))
